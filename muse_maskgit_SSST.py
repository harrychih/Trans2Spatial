...
class Transformer(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.mask_id = num_cells if add_mask_id else None
        
        self.num_cells = num_cells
        self.token_emb = nn.Embedding(num_cells + int(add_mask_id), dim)
        self.pos_emb = nn.Embedding(seq_len, dim)
        self.seq_len = num_cells

        self.transformer_blocks = TransformerBlocks(dim = dim, **kwargs)
        self.norm = LayerNorm(dim)

        self.dim_out = default(dim_out, num_cells)
        ...
        self.encode_scRNA_count = scRNASeqEmbedding(num_cells, gene_embed_dim)

        self.gene_embed_proj = nn.Linear(num_cells, dim, bias = False) if gene_embed_dim != dim else nn.Identity() 
    ...
    def forward(
        ...
        cond_drop_prob = 0.,
        conditioning_gene_ids: Optional[torch.Tensor] = None,
        scRNA_count: Optional[torch.Tensor] = None,
        gene_embeds: Optional[torch.Tensor] = None
    ):
        ...
        assert exists(scRNA_count) ^ exists(gene_embeds)

        if exists(scRNA_count):
            gene_embeds = self.encode_scRNA_count(scRNA_count)

        context = self.gene_embed_proj(gene_embeds)

        context_mask = (gene_embeds != 0).any(dim = -1)
        ...
        if exists(conditioning_gene_ids):
            conditioning_gene_ids = rearrange(conditioning_gene_ids, 'b ... -> b (...)')
            cond_gene_emb = self.token_emb(conditioning_gene_ids)
            context = torch.cat((context, cond_gene_emb), dim = -2)
            context_mask = F.pad(context_mask, (0, conditioning_gene_ids.shape[-1]), value = True)
...
class MaskGit(nn.Module):
    def __init__(
        ...
        assert self.vae.codebook_size == self.cond_vae.codebook_size == transformer.num_cells, 'transformer num_cells must be set to be equal to the vae codebook size'
    ...
    def generate(
        ...
        gene_embeds = scRNASeqEmbedding(texts)
    ...
    def forward(
        ...
        if exists(texts):
            text_embeds = self.transformer.encode_scRNA_count(texts).to(device)
            texts = None
        ...
        if self.transformer.self_cond and random() < self.self_cond_prob:
            with torch.no_grad():
                _, self_cond_embed = self.transformer(
                    x,
                    gene_embeds = text_embeds,
                    conditioning_gene_ids = cond_token_ids,
                    cond_drop_prob = 0.,
                    return_embed = True
                )
